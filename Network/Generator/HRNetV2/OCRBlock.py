#  https://github.com/openseg-group/openseg.pytorch/blob/master/lib/models/nets/hrnet.py
import os
import pdb
import math
import torch
...
class _ObjectAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.f_pixel = nn.Sequential(
            ...
            LayerHelper.get_norm_layer(num_features=self.key_channels, norm_type=norm_type),
            nn.ReLU(),
            # ModuleHelper.BNReLU(self.key_channels, bn_type=norm_type),
        ...
        self.f_up = nn.Sequential(
            nn.Conv2d(in_channels=self.key_channels, out_channels=self.in_channels,
                      kernel_size=1, stride=1, padding=0),
            LayerHelper.get_norm_layer(num_features=self.in_channels, norm_type=norm_type),
            nn.ReLU(),
            # ModuleHelper.BNReLU(self.in_channels, bn_type=norm_type),
        )
...
class SpatialOCR_Module(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_bn_dropout = nn.Sequential(
            ...
            nn.ReLU(),
            # ModuleHelper.BNReLU(out_channels, bn_type=norm_type),
...
class SpatialOCR_Context(nn.Module):
    ...
    We aggregate the global object representation to update the representation for each pixel.
    """
    def __init__(self, in_channels, key_channels, scale=1, dropout=0, norm_type=None):
...
class SpatialOCR_ASP_Module(nn.Module):
    def __init__(self, features, hidden_features=256, out_features=512, dilations=(12, 24, 36), num_classes=19, norm_type=None, dropout=0.1, padding_type="reflect"):
        super(SpatialOCR_ASP_Module, self).__init__()
        # from lib.models.modules.spatial_ocr_block import SpatialOCR_Context
        self.context = nn.Sequential(nn.Conv2d(features, hidden_features, kernel_size=3, padding=1, dilation=1, bias=True, padding_mode=padding_type),
                                     LayerHelper.get_norm_layer(num_features=hidden_features, norm_type=norm_type),
                                     nn.ReLU(),

                                     # ModuleHelper.BNReLU(hidden_features, bn_type=norm_type),
                                     SpatialOCR_Context(in_channels=hidden_features,
                                                        key_channels=hidden_features//2, scale=1, norm_type=norm_type),
                                     )
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(features, hidden_features, kernel_size=3, padding=dilations[0], dilation=dilations[0], bias=True, padding_mode=padding_type),
                                   LayerHelper.get_norm_layer(num_features=hidden_features, norm_type=norm_type),
                                   nn.ReLU(),
                                   # ModuleHelper.BNReLU(hidden_features, bn_type=norm_type),
                                   )
        self.conv4 = nn.Sequential(nn.Conv2d(features, hidden_features, kernel_size=3, padding=dilations[1], dilation=dilations[1], bias=True, padding_mode=padding_type),
                                   LayerHelper.get_norm_layer(num_features=hidden_features, norm_type=norm_type),
                                   nn.ReLU(),
                                   # ModuleHelper.BNReLU(hidden_features, bn_type=norm_type),
                                   )
        self.conv5 = nn.Sequential(nn.Conv2d(features, hidden_features, kernel_size=3, padding=dilations[2], dilation=dilations[2], bias=True, padding_mode=padding_type),
                                   LayerHelper.get_norm_layer(num_features=hidden_features, norm_type=norm_type),
                                   nn.ReLU(),
                                   # ModuleHelper.BNReLU(hidden_features, bn_type=norm_type),
                                   )
        self.conv_bn_dropout = nn.Sequential(
            ...
            LayerHelper.get_norm_layer(num_features=out_features, norm_type=norm_type),
            nn.ReLU(),
            # ModuleHelper.BNReLU(out_features, bn_type=norm_type),
            nn.Dropout2d(dropout)
        )
